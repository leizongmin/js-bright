...
syntax.parse = function (words, isNested) {
  ...
  if (!isNested) {
    ...
    if (context.defers.length > 0) {
      ...
      defersCode.push(indent + '$$_callback = function ($$_err) {')
      defersCode.push(indent + '  var next = function () {')
      defersCode.push(indent + '    var fn = $$_defers.shift();')
      defersCode.push(indent + '    if (typeof(fn) !== "function") return $$_callback($$_err);')
      defersCode.push(indent + '    fn(function (err) {')
      defersCode.push(indent + '      if (err) console.error(err && err.stack);')
      defersCode.push(indent + '      next();')
 ...
 * @param {Object} context
 */
syntax.parseNested = function (context) {
