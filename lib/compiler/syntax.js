 ...
 * @param {String} line
 */
var codePushLine = function (context, line) {
  // 生成缩进空格
...
syntax.parse = function (tokenList, isNested) {
  ...
  var addLineNumber = function (t, msg) {
    codePushLine(context, '/* LINE:' + (t.line + 1) + ' ' + msg + ' */');
  };
  ...
  for (var ret; ret = readLine(context.tokenList);) {
    ...
    if (firstT.type === TOKEN.BLANK) {
      ...
      switch (firstT.text) {
        ...
        case 'break':
          addLineNumber(firstT, 'START');
          codePushLine(context, 'return $$_break(null);');
          addLineNumber(firstT, 'END');
          break;

        case 'continue':
          addLineNumber(firstT, 'START');
          codePushLine(context, 'return $$_continue(null);');
          addLineNumber(firstT, 'END');
          break;
...
syntax.parseAwait = function (context, names, tokenList) {
  ...
  codePushLine(context, call);
  context.indent++;
  if (names.length > 0) {
    code.init.forEach(function (line) {
      codePushLine(context, line);
    });
  }
  syntax.parse(context, true);
  context.indent--;
  codePushLine(context, '});');
};
...
syntax.parseReturn = function (context, tokenList, isNested) {
  ...
  codePushLine(context, 'return $$_callback' + (isNested ? '' : '_global') + '(' + values.join(', ') + ');');
};
...
syntax.parseThrow = function (context, tokenList) {
  if (tokenList.length < 1) {
    codePushLine(context, 'return $$_callback_global(new Error());');
  } else {
    syntax.parseExpression(context, 'var $$_err', tokenList);
    codePushLine(context, 'return $$_callback_global($$_err);');
  }
};
