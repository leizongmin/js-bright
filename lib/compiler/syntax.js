 ...
 * @param {String} e
 */
syntax.throwError = function (t, e) {
  if (!e) {
 ...
 * @return {Object}
 */
syntax.newToken = function (type, line, column, text) {
...
syntax.addReturnAtEnd = function (tokenList, isNested) {
  ...
  if (lastT) {
    var tReturn = syntax.newToken(TOKEN.KEYWORD, lastT.line + 1, 0, 'return');
    if (isNested) {
...
syntax.parseCondition = function (tokenList) {
  ...
  if (!(lastT.type === TOKEN.SYMBLE && lastT.text === '{')) {
    return syntax.throwError(lastT);
  }
  // 取出条件部分
  tokenList.pop();
  var cond = '';
        ...
        if (t.type === TOKEN.SYMBLE) {
          if (t.text === '[' || t.text === '(') {
            ...
            if (!((t.text === ']' && b === '[') || (t.text === ')' && b === '('))) {
              return syntax.throwError(t, 'brackets do not match');
            }
          }
        }
      });
  ...
  for (var ret; ret = syntax.readLine(context.tokenList);) {
    ...
    if (firstT.type === TOKEN.BLANK) {
      ...
      var needNextToken = function () {
        if (nextTs.length < 1) {
          return syntax.throwError(firstT, 'Unexpected end of input');
        }
      };
      ...
      switch (firstT.text) {
        ...
        case 'javascript':
          needNextToken();
          addLineNumber(firstT, 'START');
          syntax.parseJavascript(context, nextTs);
          addLineNumber(firstT, 'END');
          break;

        default:
          return syntax.throwError(firstT);
      }
    } else {
      addLineNumber(firstT, 'START');
      // 其他语句，直接返回原来的代码
 ...
 *  - {Array} next
 */
syntax.parseBraceBody = function (context) {
  ...
  while (ret = syntax.readLine(tokenList)) {
    tokenList = ret.next;
    var line = ret.line;
    var _line = syntax.noBlankToken(line);
    var firstT = _line[0];
...
syntax.parseNested = function (context, isReturn) {
  var ret = syntax.parseBraceBody(context);
  if (isReturn) {
    ret.body = syntax.addReturnAtEnd(ret.body, true);
  }
...
syntax.parseVar = function (context, tokenList) {
  ...
  var initTokens = [];
  var lastT = tokenList[tokenList.length - 1];
  tokenList.push(syntax.newToken(TOKEN.SYMBLE, lastT.line, lastT.column + lastT.text.length, ','));
  tokenList.forEach(function (t) {
    if (isInit) {
      if (t.type === TOKEN.SYMBLE) {
        if (t.text === '(' || t.text === '[') {
          ...
          if (brackets.length > 0) {
            ...
            // 初始化结束，添加初始化语句
            initTokens.push(syntax.newToken(TOKEN.SYMBLE, t.line, t.column, ';'));
...
syntax.parseLet = function (context, tokenList) {
  if (tokenList.length < 3) {
    return syntax.throwError(tokenList[tokenList.length - 1], 'Unexpected end of input');
  }
  ...
  if (tokenList[0].type === TOKEN.KEYWORD) {
    if (tokenList[0].text === 'await') {
      if (!tokenList[1]) {
        return syntax.throwError(tokenList[0], 'Unexpected end of input')
      }
      ...
      if (names.length > 1) {
        return syntax.throwError(tokenList[0], 'Not support tuple assignment');
      }
      syntax.parseFunction(context, names[0], tokenList.slice(1));
    } else {
      return syntax.throwError(tokenList[0]);
    }
  } else {
    if (names.length > 1) {
      return syntax.throwError(tokenList[0], 'Not support tuple assignment');
...
syntax.parseSleep = function (context, tokenList) {
  var lastT = tokenList[tokenList.length - 1];
  tokenList.push(syntax.newToken(TOKEN.SYMBLE, lastT.line, lastT.column + lastT.text.length + 1, ';'));
 ...
 * @param {Array} tokenList
 */
syntax.parseJavascript = function (context, tokenList) {
  var firstT = tokenList[0];
  ...
  var line;
  while (line = syntax.readLine(tokenList)) {
    tokenList = line.next;
    var isName = false;
    var code = '';
