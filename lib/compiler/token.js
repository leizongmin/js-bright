...
exports.parse = function (source) {
      ...
      switch (w.type) {
        ...
        case WORD.NAME:
          if (isKeyword(w.text)) {
            ...
            var prevW = words[i - 1];
            var nextW = words[i + 1];
            prevW = prevW && prevW.type === WORD.SYMBLE ? prevW.text : null;
            nextW = nextW && nextW.type === WORD.SYMBLE ? nextW.text : null;
            if (prevW === '.' || nextW === '.') {
              w.type = WORD.IDENTIFIER;
            } else if ((prevW === '{' || prevW === ',') && nextW === ':') {
              w.type = WORD.IDENTIFIER;
            } else {
              w.type = WORD.KEYWORD;
            }
        ...
        case WORD.NUMBER:
          ...
          var prevW = words[i - 1];
          if (prevW && prevW.type === WORD.SYMBLE && prevW.text === '.') {
            if (prevW.column + 1 === w.column && prevW.line === w.line) {
              var prevW2 = words[i - 2];
              if (!prevW2 || prevW2.type === WORD.SYMBLE) {
                var newW = {
                  type:   WORD.NUMBER,
                  text:   prevW.text + w.text,
                  line:   prevW.line,
                  column: prevW.column
                };
                words[i - 1] = newW;
        ...
        case WORD.SYMBLE:
          // 合并两个符号的操作符，如：++ -- << >> == >= <= !=
          var nextW = words[i + 1];
          if (nextW && w.line === nextW.line) {
            if ((w.text === '+' && nextW.text === '+') ||
                (w.text === '-' && nextW.text === '-') ||
                (w.text === '<' && nextW.text === '<') ||
                (w.text === '>' && nextW.text === '>') ||
                (w.text === '=' && nextW.text === '=') ||
  ...
  words.forEach(function (w) {
    if (w.type === WORD.KEYWORD && list.indexOf(w.text) !== -1) {
      w.type = WORD.IDENTIFIER;
    }
  });
