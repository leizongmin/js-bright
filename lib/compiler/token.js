      ...
      switch (w.type) {
        ...
        case TOKEN.NAME:
          if (isKeyword(w.text)) {
            ...
            var prevW = words[i - 1];
            var nextW = words[i + 1];
            prevW = prevW && prevW.type === TOKEN.SYMBLE ? prevW.text : null;
            nextW = nextW && nextW.type === TOKEN.SYMBLE ? nextW.text : null;
            if (prevW === '.' || nextW === '.') {
        ...
        case TOKEN.NUMBER:
          ...
          var prevW = words[i - 1];
          if (prevW && prevW.type === TOKEN.SYMBLE && prevW.text === '.') {
            if (prevW.column + 1 === w.column && prevW.line === w.line) {
              var prevW2 = words[i - 2];
              if (!prevW2 || prevW2.type === TOKEN.SYMBLE) {
                var newW = {
                  type:   TOKEN.NUMBER,
